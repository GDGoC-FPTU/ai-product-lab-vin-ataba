...
Nhóm chọn **GO** vì bài toán có giá trị vận hành rõ ràng, workflow hiện tại có bottleneck cụ thể, và giải pháp AI có thể triển khai trước ở phạm vi hẹp dưới dạng công cụ hỗ trợ nội bộ. Về mặt kỹ thuật, AI vision và LLM có thể giúp giảm đáng kể thời gian khoanh vùng trẻ lạc, trong khi rủi ro vẫn được kiểm soát bằng Human-in-the-loop: AI chỉ gợi ý khu vực/camera nghi vấn, con người luôn xác nhận trước khi hành động.


Quyết định hợp lý là bắt đầu xây dựng prototype nội bộ với phạm vi hẹp:
...
## Điều kiện triển khai GO có kiểm soát


* Prototype chỉ chạy nội bộ trong một Vincom/kịch bản thử nghiệm, chưa triển khai đại trà.
* Có baseline hiện tại: số ca/tháng, thời gian tìm trung bình, số nhân sự tham gia, tỉ lệ phát hiện qua camera.
* Có bộ dữ liệu test an toàn: mô phỏng, diễn tập có đồng ý rõ ràng hoặc dữ liệu đã ẩn danh.
...
Dự án được đánh giá **GO với scope hẹp**: bắt đầu xây dựng prototype nội bộ để hỗ trợ nhân viên Vincom khoanh vùng trẻ lạc nhanh hơn, nhưng chưa tự động hóa quyết định cuối cùng. Với ranh giới vận hành rõ ràng, dữ liệu test an toàn và quy trình Human-in-the-loop bắt buộc, giải pháp này có tiềm năng giảm thời gian tìm kiếm, giảm áp lực cho phụ huynh và giúp nhân viên xử lý sự cố có hệ thống hơn.
